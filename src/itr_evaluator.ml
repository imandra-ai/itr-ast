...
let reconstruct_ors (es : record_item list) : record_item =
  let rec reconstruct_ors_rec (es : record_item list) : expr =
    ...
    | [ h ] -> ( match h with Rec_value h -> h | _ -> expr_f)
    | lhs :: t -> (
        if CCList.mem lhs t then reconstruct_ors_rec t
        else
          match lhs with
...
let reconstruct_ands (es : record_item list) : record_item =
  let rec reconstruct_ands_rec (es : record_item list) : expr =
    ...
    | [ h ] -> ( match h with Rec_value h -> h | _ -> expr_f)
    | lhs :: t -> (
        if CCList.mem lhs t then reconstruct_ands_rec t
        else
          match lhs with
...
and evaluate_expr (context : 'a context) (e : expr) : record_item =
  ...
  | Or { lhs : expr; rhs : expr } ->
      ...
      if CCList.exists is_true evaluated_ors then e_true
      else if CCList.for_all is_false evaluated_ors then e_false
      else
        let simplified_ors =
  ...
  | And { lhs : expr; rhs : expr } ->
      ...
      if CCList.exists is_false evaluated_ands then e_false
      else if CCList.for_all is_true evaluated_ands then e_true
      else
        let simplified_ands =
  ...
  | Value (Funcall { func : string; args : record_item list })
    when func = "defaultIfNotSet" && List.length args = 2 -> (
      ...
      | [ d; a ] -> (
          ...
          | _ ->
              if is_ground a && (not @@ is_none a) then a
              else
                Rec_value
                  (Value
                     (Funcall
    ...
    when func = "Set.subset" && List.length args = 2 -> (
      ...
      | [ l; r ] -> (
          ...
          | ( Rec_value (Value (Literal (Coll l))),
              ...
              if CCList.subset ~eq:(fun a b -> a = b) l r then e_true
              else e_false
          | _l, _r ->
  ...
  | Eq
      ...
      then []
        ...
        | lhs, Rec_value (Value (MessageValue { var = None; field_path })) ->
            if is_false @@ evaluate_expr (Eq { lhs; rhs }) then [ field_path ]
            else []
